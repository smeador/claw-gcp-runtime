...
docker compose --env-file config/docker.build.env -f "${COMPOSE_FILE}" up -d openclaw-gateway >/dev/null

CONTAINER_ID="$(docker compose --env-file config/docker.build.env -f "${COMPOSE_FILE}" ps -q openclaw-gateway)"
...
docker compose --env-file config/docker.build.env -f "${COMPOSE_FILE}" exec -T --user root openclaw-gateway bash -lc '
  set -euo pipefail
...
docker compose --env-file config/docker.build.env -f "${COMPOSE_FILE}" exec -T openclaw-gateway bash -lc "
  set -euo pipefail
