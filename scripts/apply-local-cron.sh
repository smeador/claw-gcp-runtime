...
CONFIG_PATH="${1:-config/cron.local.json}"
...
wait_for_gateway() {
  ...
  for ((attempt = 1; attempt <= attempts; attempt += 1)); do
    if compose_cmd -f docker/compose.local.yml exec -T openclaw-gateway \
  ...
  echo "Local gateway did not become ready for cron operations after ${attempts} attempts." >&2
  return 1
}
