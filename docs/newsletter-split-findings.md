...
- the extracted newsletter repo is now bootstrapped as a sibling checkout:
  - `/Users/sean/Repos/agent-email-digest`
...
- runtime-facing entry points in `scripts/email/` and `scripts/gmail/` now resolve to:
  ...
  2. sibling `agent-email-digest` checkout
...
Once the sibling `agent-email-digest` checkout exists, the runtime repo can find it even before its dependencies are installed.
...
- `AGENT_EMAIL_DIGEST_ROOT=/path/to/agent-email-digest`
