...
- if `gog.serviceAccounts["automation@example.com"]` is present, `./scripts/prepare-local-docker.sh` also renders `config/rendered/gog-service-account.json`
...
- Bootstrap OpenAI model auth into cloud runtime state:
  ```bash
  cd /Users/sean/Repos/gcp-claw-lab
  bash ./scripts/models/bootstrap-openai-cloud.sh claw-runtime-vm claw-runtime-example us-central1-a
  ```
...
- Gmail auth split:
  - native local may keep using user OAuth for `automation@example.com`
  - Docker-local and cloud should prefer Google Workspace service-account auth for `automation@example.com`
...
- Docker Gmail automatic bootstrap:
  - paste the Workspace service-account JSON object into `config/secrets.local.json` under `gog.serviceAccounts["automation@example.com"]`
  - rerun:
    ```bash
    bash ./scripts/prepare-local-docker.sh
    docker compose -f docker/compose.local.yml up -d --force-recreate openclaw-gateway
...
- Docker Gmail one-shot bootstrap helper:
  ...
  ```
  This command starts the Docker-local gateway if needed and configures `gog` inside the container to use the Workspace service account for `automation@example.com`.
- Cloud Gmail bootstrap helper:
  ...
  bash ./scripts/gmail/bootstrap-gog-cloud-service-account.sh \
    ...
    automation@example.com
  ```
  This copies the service-account key to the VM, installs it into the cloud runtime state, and configures `gog` inside the cloud container to impersonate `automation@example.com`.
