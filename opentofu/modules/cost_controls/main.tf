...
resource "google_service_account" "function" {
  ...
  display_name = "Agent Lab budget shutdown function"
}

resource "google_project_iam_custom_role" "vm_stopper" {
  project     = var.project_id
  role_id     = "agentLabVmStopper"
  title       = "Agent Lab VM Stopper"
  description = "Stops the single Agent Lab VM when budget thresholds are exceeded."
  permissions = [
...
resource "google_cloudfunctions2_function" "shutdown" {
  ...
  location    = var.region
  description = "Stops the Agent Lab VM when a billing budget threshold is exceeded."
